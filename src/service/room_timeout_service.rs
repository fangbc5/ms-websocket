...
pub struct RoomTimeoutService {
    ...
    /// 房间ID -> 超时任务
    timeout_tasks: Arc<RwLock<HashMap<i64, TimeoutTask>>>,
}
...
impl RoomTimeoutService {
    ...
    /// 检查房间是否已关闭
    pub async fn is_close(&self, room_id: i64) -> anyhow::Result<bool> {
    ...
    /// 刷新房间活跃时间，5分钟无活动自动清理
    pub async fn refresh_room_activity(&self, room_id: i64) -> anyhow::Result<()> {
        self.schedule_room_cleanup(room_id, 300).await
    }

    /// 取消房间超时任务
    pub async fn cancel_timeout_task(&self, room_id: i64) {
    ...
    pub async fn clean_room(
        ...
        if !members.is_empty() {
            ...
            let resp = WsBaseResp::from_data("RoomClosed".to_string(), closed_vo)?;

            self.push_service.send_push_msg(resp, members, 0).await?;
        }
    ...
    pub async fn schedule_call_timeout(
        ...
        let cancel_handle = tokio::spawn(async move {
            ...
            let timeout_vo = CallTimeoutVO {
                target_uid: receiver.to_string(),
            };
            let resp = WsBaseResp::from_data("TIMEOUT".to_string(), timeout_vo).unwrap();
            let _ = push_service
                .send_push_msg_single(resp, caller, receiver)
                .await;
