...
impl RoomMetadataService {
    ...
    /// 打开房间
    pub async fn open_room(&self, room_id: i64) -> anyhow::Result<()> {
    ...
    /// 检查房间是否已关闭
    pub async fn is_room_closed(&self, room_id: i64) -> anyhow::Result<bool> {
        let mut conn = self.app_state.redis()?;
        let key = CloseRoomCacheKeyBuilder::builder(room_id);
    ...
    /// 标记房间已关闭
    pub async fn mark_room_closed(&self, room_id: i64) -> anyhow::Result<()> {
        // 清除开始时间
    ...
    /// 获取房间创建者
    pub async fn get_room_creator(&self, room_id: i64) -> anyhow::Result<Option<i64>> {
        self.get_room_metadata(room_id, "creator").await
    }

    /// 设置房间创建者
    pub async fn set_room_creator(&self, room_id: i64, creator_uid: i64) -> anyhow::Result<()> {
        self.set_room_metadata(room_id, "creator", Some(creator_uid))
            .await
    }

    /// 获取房间接通电话时间
    pub async fn get_room_start_time(&self, room_id: i64) -> anyhow::Result<Option<i64>> {
        self.get_room_metadata(room_id, "startTime").await
    }

    /// 设置房间接通电话时间
    pub async fn set_room_start_time(&self, room_id: i64) -> anyhow::Result<()> {
    ...
    /// 添加房间管理员
    pub async fn add_room_admin(&self, room_id: i64, admin_uid: i64) -> anyhow::Result<()> {
        let mut conn = self.app_state.redis()?;
    ...
    /// 获取房间媒体类型
    pub async fn get_room_medium_type(&self, room_id: i64) -> anyhow::Result<Option<bool>> {
        self.get_room_metadata(room_id, "mediumType").await
    }

    /// 设置房间媒体类型
    pub async fn set_room_medium_type(&self, room_id: i64, is_video: bool) -> anyhow::Result<()> {
        self.set_room_metadata(room_id, "mediumType", Some(is_video))
            .await
    }

    /// 设置租户 ID
    pub async fn set_tenant_id(&self, room_id: i64, tenant_id: i64) -> anyhow::Result<()> {
        self.set_room_metadata(room_id, "tenantId", Some(tenant_id))
            .await
    }

    /// 获取租户 ID
    pub async fn get_tenant_id(&self, room_id: i64) -> anyhow::Result<Option<i64>> {
        self.get_room_metadata(room_id, "tenantId").await
    }

    /// 设置房间类型
    pub async fn set_room_type(&self, room_id: i64, r#type: i32) -> anyhow::Result<()> {
        self.set_room_metadata(room_id, "type", Some(r#type)).await
    }

    /// 获取房间类型
    pub async fn get_room_type(&self, room_id: i64) -> anyhow::Result<Option<i32>> {
    ...
    pub async fn get_room_admins(
        ...
        let admins: std::collections::HashSet<i64> =
            members.into_iter().filter_map(|s| s.parse().ok()).collect();
        Ok(admins)
    }

    /// 检查用户是否是房间管理员
    pub async fn is_room_admin(&self, room_id: i64, uid: i64) -> anyhow::Result<bool> {
        // 创建者也是管理员
    ...
    /// 获取全体静音状态
    pub async fn is_all_muted(&self, room_id: i64) -> anyhow::Result<bool> {
        Ok(self
    ...
    /// 获取当前屏幕共享用户 ID
    pub async fn get_screen_sharing_user(&self, room_id: i64) -> anyhow::Result<Option<i64>> {
        self.get_room_metadata(room_id, "screenSharingUser").await
    }

    /// 检查是否正在屏幕共享
    pub async fn is_screen_sharing(&self, room_id: i64) -> anyhow::Result<bool> {
